...
import com.p3lj2.koveepetshop.adapter.LogEmployeeAdapter;
...
public class EmployeeLogFragment extends Fragment {
    ...
    private LogEmployeeAdapter logAdapter;
    ...
    private void initRecyclerView() {
        logAdapter = new LogEmployeeAdapter();
    ...
    private void getData() {
        logViewModel.logEmployee(employee.getToken()).observe(getViewLifecycleOwner(), new Observer<List<EmployeeModel>>() {
            @Override
            public void onChanged(List<EmployeeModel> employeeModels) {
                if (employeeModels != null) {
                    logAdapter.setItems(employeeModels);
                }
            }
        });
    }
